library(shiny)
library(tidyverse)
library(leaflet)
library(stringr)
library(geojsonio)
library(DT)


murder_table <- readRDS("murders.RData")
##Calling initial data and filtering data
source("murder_table.R")

##Calling state_function to summarize statistics for each state into the most common aspects of crimes 
source("state_function.R")

source("ggplots.R")

navbarPage(title = "Murder Trends in the U.S. from 1980 to 2014",
           theme = shinythemes::shinytheme("sandstone"),
           header = tags$style(HTML("
                                    .leaflet-container {
                                    background: #BAECFF;
                                    outline: 0;
                                    }"
                                    )),
           tags$style(type = "text/css", ".navbar-brand {font-size: 36px;}"),
           tabPanel("Map",
                    tags$h2("What are the characteristics around murder incidences pertinent to each state across 1980 to 2014?"),
                    mainPanel(leafletOutput("StateMap", width = 900, height = 600),
                              sliderInput("range","Year", min=1980, max=2014, value = 1980, step = 1)
                    )),
                    tags$h3("The homocide rate doubled from the early 1960s to the 1970s, increasing from 4.6 per 100,000 US residents to 9.7 per 100,000. In 1980 the rate peaked and then again in 1991. Since then the homocide rate has dramatically declined until 2010. Our website hopes to visualize these trends of decline and the characteristics surrounding the perpertrators and victims."),
           tabPanel("Murder Incidence by State and Year",
                    tags$h2( "How have murder incidences per month quantitatively changed across the years?"),
                    plotOutput("barplot", width = 1200, height = 600), 
                    fluidRow(
                      column(width = 2, offset = 0,
                        selectInput("Year", "Year", choices = unique(murderbargraph2$Year))), 
                      column(width = 2, offset = 0,
                        selectInput("State", "State", choices = unique(murderbargraph2$State))))),
           tabPanel("Weapon Pie Graph",
                    tags$h2("What is the most common murder weapon nation-wide each year between 1980 to 2010 in comparison to the average across this entire time period?"),
                    splitLayout(plotOutput("weaponpiechartinteractive", width = 600, height = 600), 
                                plotOutput("weaponpiechart", width = 600, height = 600)),
                    selectInput("Year", "Year", choices = unique(common_weapon_year$Year),
                     )),
           tabPanel("Murder Incidence Through the Years",
                    tags$h2("How has the nation trend of murder rates changed from 1980 to 2010?"),
                    plotOutput("scatterplot", width = 1200, height = 600)),
           tabPanel("Race Pie Graph",
                    tags$h2("What is the race distribution of victims and perpetrators from 1980 to 2010?"),
                    splitLayout(plotOutput("racepiechart", width = 600, height = 600), plotOutput("perpracepiechart", width = 600, height = 600))),
                    tags$h3("The visualization concerning the victim race is located on the left, and the visualization concerning the perpetrator is located on the right of the page."),
           tabPanel("Raw Data", DT::dataTableOutput("mytable")),
                    downloadButton("downloadData", "Download"))
           tabPanel("About the Creators",
                    tags$h2("Meet the Creators!"),
                    tags$img(src = "creators.jpg", width = 600),
                    tags$figcaption("Lucy Worthy: , Mason Davis: Neuroscience Major with Minors in POV and Data Science, Gretta Syrett, Anna Marie Harding: Neuroscience Major")
                    )
